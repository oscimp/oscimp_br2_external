...
define POCKETSDR_BUILD_CMDS
    ...
    $(INSTALL) -D -m 0755 $(@D)/lib/build/librtk.a  $(STAGING_DIR)/usr/lib/
    $(INSTALL) -D -m 0755 $(@D)/lib/build/librtk.so $(STAGING_DIR)/usr/lib/
    $(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(@D)/lib/build -f libsdr.mk
    $(INSTALL) -D -m 0755 $(@D)/lib/build/libsdr.a  $(STAGING_DIR)/usr/lib/
    $(INSTALL) -D -m 0755 $(@D)/lib/build/libsdr.so  $(STAGING_DIR)/usr/lib/
...
define POCKETSDR_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -D -m 0755 $(@D)/lib/build/librtk.a $(TARGET_DIR)/usr/lib
	$(INSTALL) -D -m 0755 $(@D)/lib/build/librtk.so $(TARGET_DIR)/usr/lib
	$(INSTALL) -D -m 0755 $(@D)/lib/build/libsdr.a $(TARGET_DIR)/usr/lib
	$(INSTALL) -D -m 0755 $(@D)/lib/build/libsdr.so $(TARGET_DIR)/usr/lib
endef
